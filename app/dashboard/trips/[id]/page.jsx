            ...
            <>
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-8">
                <div className="min-w-0">
                  <p className="text-[12px] uppercase tracking-wide text-slate-400 dark:text-slate-500 mb-1">Trip to</p>
              ...
              <div className="flex flex-wrap gap-2 mb-10">
                {trip.interests?.map((i) => (
                  <span key={i} className="text-[12px] font-semibold text-teal-700 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/30 px-3 py-1 rounded-full">
                    {i}
                  </span>
                ))}
              </div>
                ...
                <div className="space-y-8">
                  {trip.aiResult.itinerary && (
                    <Section title="Overview">
                      <p className="text-[14px] sm:text-[14.5px] leading-relaxed text-slate-600 dark:text-slate-300">{trip.aiResult.itinerary}</p>
                    </Section>
                  )}
                    ...
                    <Section title="Daily schedule">
                      <div className="space-y-4">
                        {trip.aiResult.dailySchedule.map((day) => (
                          <div key={day.day} className="rounded-2xl border border-slate-100 dark:border-slate-800 p-4 sm:p-5">
                            <p className="text-[12px] font-bold text-teal-600 dark:text-teal-400 uppercase tracking-wide mb-3">Day {day.day}</p>
                  ...
                  <div className="grid sm:grid-cols-2 gap-6">
                    {trip.aiResult.estimatedExpenses && (
                      <Section title="Estimated expenses">
                        <p className="text-[14px] text-slate-600 dark:text-slate-300">{trip.aiResult.estimatedExpenses}</p>
                      </Section>
                    )}
                    {trip.aiResult.bestVisitingTime && (
                      <Section title="Best time to visit">
                        <p className="text-[14px] text-slate-600 dark:text-slate-300">{trip.aiResult.bestVisitingTime}</p>
                      </Section>
                    )}
                    {trip.aiResult.localTransportation && (
                      <Section title="Local transportation">
                        <p className="text-[14px] text-slate-600 dark:text-slate-300">{trip.aiResult.localTransportation}</p>
                      </Section>
                    )}
                  </div>
                    ...
                    <Section title="Packing checklist">
                      <div className="grid sm:grid-cols-2 gap-2">
                        {trip.aiResult.packingChecklist.map((item, idx) => (
                          <div key={idx} className="flex items-center gap-2.5 text-[13.5px] text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 rounded-lg px-3 py-2">
                            <svg viewBox="0 0 24 24" fill="none" stroke="#06B6D4" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5 shrink-0">
...
function Stat({ label, value, capitalize }) {
  return (
    <div className="bg-slate-50 dark:bg-slate-900 rounded-xl px-3 sm:px-4 py-3 text-center">
      <p className={`text-[14px] sm:text-[15px] font-bold truncate ${capitalize ? 'capitalize' : ''}`}>{value}</p>
      <p className="text-[10.5px] sm:text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">{label}</p>
    </div>
  );
}
