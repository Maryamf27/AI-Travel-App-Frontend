        ...
        <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-[13.5px] sm:text-[14px] text-slate-700 dark:text-slate-400 mt-1">
          ...
          {loading ? (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="h-48 rounded-2xl bg-slate-50 dark:bg-slate-900 animate-pulse" />
              ))}
            </div>
          ) : trips.length === 0 ? (
            ...
            <>
              ...
              {insights && (
                <section className="mt-10 sm:mt-12">
                  <h2 className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-3">
                    Travel insights
                  </h2>
...
function EmptyState({ onCreate }) {
  return (
    <div className="text-center py-16 sm:py-20 px-4 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
      ...
      <p className="text-[13.5px] sm:text-[14px] text-slate-500 dark:text-slate-400 mb-6">Create your first AI-generated itinerary in under a minute.</p>
      <button
        onClick={onCreate}
