...
import { Users, MapPin, Briefcase, Compass, UserCog, Settings, Activity, DollarSign, Crown } from 'lucide-react';
...
export default function AdminDashboardPage() {
  ...
  return (
    <div>
      {/* Welcome card */}
      <div className="rounded-2xl bg-linear-to-br from-cyan-500 via-purple-500 to-pink-600 dark:from-cyan-700 dark:via-purple-700 dark:to-pink-800 p-6 sm:p-8 mb-8 text-white">
      ...
      {/* Stats */}
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 mb-5">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 mb-10">
        <StatCard
          icon={DollarSign}
          label="Revenue (paid subscriptions)"
          value={
            stats
          ...
          hint={stats ? `${stats.totalPaidPayments ?? 0} payments` : undefined}
          loading={loading}
        />
        <StatCard icon={Crown} label="Premium Subscribers" value={stats?.totalPremiumUsers ?? 0} loading={loading} />
      </div>
            ...
            {QUICK_ACTIONS.map((action) => {
              ...
              return (
                <Link
                  key={action.href}
                  href={action.href}
                  className="flex items-center gap-3 rounded-xl border border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 px-4 py-3.5 hover:border-purple-200 dark:hover:border-purple-900/50 hover:shadow-sm transition"
                >
                  <div className="w-9 h-9 rounded-lg bg-purple-50 dark:bg-purple-950/30 flex items-center justify-center shrink-0">
                    <Icon className="w-4.5 h-4.5 text-purple-600 dark:text-purple-400" strokeWidth={2.2} />
                  </div>
