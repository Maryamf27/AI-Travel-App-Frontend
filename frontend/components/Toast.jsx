      ...
      <div
        className={`flex items-start gap-3 rounded-2xl border px-4 py-3.5 shadow-2xl backdrop-blur ${
          isError
            ...
            : 'bg-white/95 dark:bg-zinc-900/95 border-zinc-100 dark:border-zinc-800'
        }`}
      >
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
            isError
              ...
              : 'bg-linear-to-r from-cyan-500 via-purple-500 to-pink-600 dark:from-cyan-600 dark:via-purple-600 dark:to-pink-700'
          }`}
        >
          {isError ? (
        ...
        <p className={`text-[13.5px] font-medium leading-snug pt-1 ${isError ? 'text-red-700 dark:text-red-300' : 'text-zinc-700 dark:text-zinc-200'}`}>
          {toast.message}
        </p>
        <button
          onClick={onClose}
          className="ml-auto text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 transition shrink-0 mt-0.5"
          aria-label="Dismiss"
        >
