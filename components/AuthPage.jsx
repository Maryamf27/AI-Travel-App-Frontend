...
function AuthPageContent() {
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-[1.05fr_1fr] bg-white dark:bg-slate-950 transition-colors">
      {/* Brand Panel */}
      <div
      ...
      >
        <div className="pointer-events-none absolute inset-0 bg-linear-to-br from-[#020617]/85 via-[#0E7490]/75 to-[#020617]/90" />
        <div
          className="pointer-events-none absolute inset-0 opacity-50"
          style={{ background: 'radial-gradient(120% 120% at 15% 0%, rgba(6,182,212,0.45) 0%, transparent 45%)' }}
        />
        <div className="relative flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-[9px] bg-teal-500 flex items-center justify-center">
          ...
          <span className="text-[17px] font-semibold text-white">
            Travel<span className="text-teal-400">AI</span>
          </span>
        </div>
        ...
        <div className="relative max-w-105">
          ...
          <h1 className="text-3xl xl:text-4xl font-semibold leading-tight text-white mb-4">
            Plan smarter.<br />Travel <span className="text-teal-400">safer.</span>
          </h1>
      ...
      <div className="relative flex flex-col items-center justify-center px-5 sm:px-6 py-10 sm:py-7 lg:px-9">
        ...
        <div className="w-full max-w-95">
          <div className="flex bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-1 mb-5 sm:mb-6">
            <button
              onClick={() => { setMode('login'); setError(''); }}
              className={`flex-1 py-2.5 rounded-[9px] text-[13.5px] font-semibold transition ${mode === 'login' ? 'bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-50 shadow' : 'text-slate-500 dark:text-slate-400'
                }`}
            >
              Sign in
            </button>
            <button
              ...
              className={`flex-1 py-2.5 rounded-[9px] text-[13.5px] font-semibold transition ${mode === 'signup' ? 'bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-50 shadow' : 'text-slate-500 dark:text-slate-400'
                }`}
            >
              Create account
            </button>
          ...
          {mode === 'login' ? (
            <div>
              ...
              <p className="text-[13.5px] sm:text-sm text-slate-500 dark:text-slate-400 mb-6 sm:mb-7">Sign in to continue planning your next trip.</p>
              ...
              <p className="text-center mt-6 text-[13.5px] text-slate-500 dark:text-slate-400">
            ...
            <div>
              ...
              <p className="text-[13.5px] sm:text-sm text-slate-500 dark:text-slate-400 mb-6 sm:mb-7">Start planning AI-powered trips in minutes.</p>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[{ value: 'traveler', label: 'Traveler' }, { value: 'travel_agent', label: 'Travel Agent' }].map((r) => (
                      <button
                        ...
                        className={`py-2.5 rounded-[10px] text-[13.5px] font-semibold border transition ${signupForm.role === r.value ? `${BTN_GRADIENT} border-transparent` : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300'
                          }`}
                      >
                        {r.label}
                      </button>
...
function Field({ label, children }) {
  return (
    <div>
      <label className="block text-[12.5px] font-semibold text-slate-900 dark:text-slate-100 mb-1.5">{label}</label>
      {children}
    </div>
  );
}
...
function EyeToggle({ show, onClick }) {
  return (
    <button type="button" onClick={onClick} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500">
