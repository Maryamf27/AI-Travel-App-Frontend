...
export default function LandingPage() {
  ...
  return (
    <div className="bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 transition-colors overflow-x-hidden">
...
function Nav({ menuOpen, setMenuOpen }) {
  const links = ['Features', 'Roles', 'Dashboard'];
  return (
    <header className="sticky top-0 z-50 backdrop-blur bg-white/80 dark:bg-zinc-950/80 border-b border-zinc-100 dark:border-zinc-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {links.map((l) => (
            <a key={l} href={`#${l.toLowerCase()}`} className="text-[14px] font-medium text-zinc-600 dark:text-zinc-400 hover:text-orange-600 dark:hover:text-orange-400 transition">
              {l}
            </a>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <Link href="/login" className="hidden sm:inline-block text-[14px] font-semibold text-zinc-700 dark:text-zinc-300 hover:text-orange-600 dark:hover:text-orange-400 transition">
            Log in
          </Link>
          <Link
            href="/signup"
          ...
          <button
            ...
            className="md:hidden w-9 h-9 flex items-center justify-center rounded-lg border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-300"
          >
      ...
      {/* mobile dropdown */}
      {menuOpen && (
        <div className="md:hidden border-t border-zinc-100 dark:border-zinc-900 px-4 sm:px-6 py-4 space-y-3 bg-white dark:bg-zinc-950">
          {links.map((l) => (
            <a
              key={l}
              href={`#${l.toLowerCase()}`}
              onClick={() => setMenuOpen(false)}
              className="block text-[14.5px] font-medium text-zinc-600 dark:text-zinc-300"
            >
              {l}
            </a>
          ))}
          <div className="flex items-center gap-3 pt-2">
            <Link href="/login" className="flex-1 text-center text-[14px] font-semibold py-2.5 rounded-[10px] border border-zinc-200 dark:border-zinc-800">
              Log in
            </Link>
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-12 sm:pt-20 pb-16 sm:pb-24 grid lg:grid-cols-2 gap-10 lg:gap-16 items-center relative">
        <div>
          ...
          <p className="text-[15px] sm:text-[16.5px] leading-relaxed text-zinc-600 dark:text-zinc-400 max-w-md mb-8 sm:mb-9">
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-4">
            ...
            <Link
              href="/login"
              className="inline-flex items-center gap-2 text-[14px] sm:text-[15px] font-semibold text-zinc-700 dark:text-zinc-300 px-2 py-3 sm:py-3.5 hover:text-orange-600 dark:hover:text-orange-400 transition"
            >
              Already have an account? Log in
            </Link>
          </div>

          <p className="text-[12px] sm:text-[12.5px] text-zinc-400 dark:text-zinc-600 mt-4 sm:mt-5">No credit card required · Free for travelers</p>
        </div>

        <div className="relative h-80 sm:h-96 lg:h-105 hidden lg:block">
          <div className="absolute inset-0 rounded-[28px] bg-linear-to-br from-orange-100 via-white to-orange-50 dark:from-zinc-900 dark:via-zinc-950 dark:to-zinc-900 border border-orange-100 dark:border-zinc-800" />

          <div className="absolute top-8 left-8 right-16 bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-100 dark:border-zinc-800 p-5 -rotate-2">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-[11px] uppercase tracking-wide text-zinc-400 dark:text-zinc-500">Trip to</p>
                <p className="text-lg font-semibold font-[Space_Grotesk]">Bali, Indonesia</p>
              </div>
              <span className="text-[11px] font-bold bg-orange-50 dark:bg-orange-950/30 text-orange-700 dark:text-orange-400 px-2.5 py-1 rounded-full">Day 2 of 5</span>
            </div>
          ...
          <div className="absolute bottom-10 right-2 bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-100 dark:border-zinc-800 p-4 w-52 rotate-3">
            <p className="text-[11px] uppercase tracking-wide text-zinc-400 dark:text-zinc-500 mb-2">AI Safety Score</p>
            ...
            <div className="h-1.5 rounded-full bg-zinc-100 dark:bg-zinc-800 overflow-hidden">
              <div className="h-full w-[92%] bg-orange-600 dark:bg-orange-600/80 rounded-full" />
            </div>
            <p className="text-[11px] text-zinc-400 dark:text-zinc-500 mt-2">Low crime · No advisories</p>
          </div>

          <div className="absolute bottom-6 left-4 bg-white dark:bg-zinc-900 rounded-xl shadow-xl border border-zinc-100 dark:border-zinc-800 px-3.5 py-2.5 flex items-center gap-2">
            <div className="w-7 h-7 rounded-full bg-orange-100 dark:bg-orange-950/40 flex items-center justify-center">
              <svg viewBox="0 0 24 24" fill="none" stroke="#EA580C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4">
                <path d="M12 1v22M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" />
              </svg>
            </div>
            <div>
              <p className="text-[10px] text-zinc-400 dark:text-zinc-500">Budget used</p>
...
function ItineraryRow({ time, title, tag }) {
  return (
    <div className="flex items-center gap-3">
      <span className="text-[11.5px] font-semibold text-zinc-400 dark:text-zinc-500 w-15.5 shrink-0">{time}</span>
      <span className="w-1.5 h-1.5 rounded-full bg-orange-500 shrink-0" />
      <span className="text-[13px] font-medium flex-1">{title}</span>
      <span className="text-[10.5px] font-semibold text-orange-700 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/30 px-2 py-0.5 rounded-full shrink-0">{tag}</span>
    </div>
  );
}
      ...
      <div className="max-w-7xl mx-auto sm:text-center text-start px-4 sm:px-6 py-8 sm:py-10 grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8">
        {stats.map(([num, label]) => (
          <div key={label}>
            ...
            <p className="text-[11.5px] sm:text-[12.5px] text-zinc-500 dark:text-zinc-400 mt-1">{label}</p>
          </div>
        ))}
      </div>
    </section>
    ...
    <section id="features" className="max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        {FEATURE_LIST.map((f) => (
          <div
            key={f.title}
            className="group rounded-2xl border border-zinc-100 dark:border-zinc-900 bg-gray-100 dark:bg-zinc-900/40 p-5 sm:p-6 hover:border-orange-300 dark:hover:border-orange-900/60 hover:shadow-lg transition"
          >
            <div className="w-10 h-10 rounded-[10px] bg-orange-50 dark:bg-orange-950/30 flex items-center justify-center mb-4 group-hover:bg-linear-to-r group-hover:from-orange-600 group-hover:to-red-600 dark:group-hover:from-orange-700 dark:group-hover:to-rose-800 transition">
...
function FeatureIcon({ name }) {
  const common = { viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 1.8, strokeLinecap: 'round', strokeLinejoin: 'round', className: 'w-5 h-5 text-orange-600 dark:text-orange-400 group-hover:text-white transition' };
  const paths = {
...
function Roles() {
  return (
    <section id="roles" className="bg-zinc-50/60 dark:bg-zinc-900/30 border-y border-zinc-100 dark:border-zinc-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
        <div className="max-w-xl mb-10 sm:mb-14">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 text-[14px] sm:text-[15px]">Travelers and travel agents each get a dedicated set of tools.</p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-5 sm:gap-6 max-w-7xl  ">
          {ROLES.map((r, i) => (
            <div
              key={r.name}
              className={`rounded-2xl p-6 sm:p-7 border transition ${
                i === 0
                  ? 'bg-zinc-900 dark:bg-linear-to-br dark:from-orange-700 dark:to-rose-900 border-zinc-900 dark:border-transparent text-white'
                  : 'bg-gray-100 dark:bg-zinc-900/40 border-zinc-100 dark:border-zinc-800'
              }`}
            >
              <span className={`inline-block text-[11px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full mb-4 ${
                i === 0 ? 'bg-white/15 text-white' : `${ACCENT_BG_SOFT} text-orange-700 dark:text-orange-400`
              }`}>
                Role {i + 1}
              </span>
              <h3 className={`text-xl font-semibold font-[Space_Grotesk] mb-1 ${i === 0 ? 'text-white' : ''}`}>{r.name}</h3>
              <p className={`text-[13.5px] mb-5 ${i === 0 ? 'text-white/70' : 'text-zinc-500 dark:text-zinc-400'}`}>{r.tagline}</p>
              <ul className="space-y-2.5">
                {r.items.map((it) => (
                  <li key={it} className="flex items-start gap-2.5 text-[13.5px]">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" className={`w-4 h-4 mt-0.5 shrink-0 ${i === 0 ? 'text-orange-200 dark:text-white' : ACCENT_TEXT}`}>
                      <path d="M5 12l4 4 10-10" />
                    </svg>
                    <span className={i === 0 ? 'text-white/90' : 'text-zinc-600 dark:text-zinc-300'}>{it}</span>
                  </li>
                ))}
              </ul>
            </div>
  ...
  return (
    <section id="dashboard" className="max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
      <div className="max-w-2xl mx-auto text-center mb-8 sm:mb-10">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 text-[14px] sm:text-[14.5px] leading-relaxed">
          Upcoming trips, budget overview, recent bookings, saved places, travel history, and AI suggestions — all on one screen.
        </p>
      </div>
      <div className="max-w-2xl mx-auto rounded-2xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40 p-5 sm:p-6 grid grid-cols-2 sm:grid-cols-3 gap-3">
        {['Upcoming trips', 'Budget overview', 'Recent bookings', 'Saved places', 'Travel history', 'AI suggestions'].map((m) => (
          <div key={m} className="flex items-center gap-2.5 text-[13px] sm:text-[13.5px] font-medium text-zinc-700 dark:text-zinc-300 bg-white dark:bg-zinc-900 rounded-xl px-3 py-2.5 border border-zinc-100 dark:border-zinc-800">
            <span className="w-1.5 h-1.5 rounded-full bg-orange-600 dark:bg-orange-500 shrink-0" />
            {m}
          </div>
        ))}
      </div>
...
function CtaBanner() {
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-16 sm:pb-24">
      <div className="relative overflow-hidden rounded-[28px] bg-linear-to-br from-[#1C0A03] via-[#7C2D12] to-[#1C0A03] px-6 sm:px-10 py-12 sm:py-16 text-center">
        <div
          className="pointer-events-none absolute inset-0"
          style={{ background: 'radial-gradient(60% 80% at 50% 0%, rgba(234,88,12,0.28) 0%, transparent 60%)' }}
        />
        ...
        <Link
          href="/signup"
          className={`relative inline-flex items-center gap-2 ${BTN_GRADIENT} text-[14.5px] sm:text-[15px] font-bold px-6 sm:px-7 py-3 sm:py-3.5 rounded-xl shadow-[0_8px_24px_rgba(234,88,12,0.3)] dark:shadow-[0_8px_24px_rgba(159,18,57,0.2)]`}
        >
          Get started for free
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4">
            <path d="M5 12h14M13 6l6 6-6 6" />
    ...
    <footer className="border-t border-zinc-100 dark:border-zinc-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-[7px] bg-linear-to-r from-orange-600 to-red-600 dark:from-orange-700 dark:to-rose-800 flex items-center justify-center">
        ...
        <p className="text-[12px] sm:text-[12.5px] text-zinc-400 dark:text-zinc-600">© {new Date().getFullYear()} TravelAI. All rights reserved.</p>
      </div>
    </footer>
  );
}
