...
export default function ProfileForm() {
  ...
  return (
    <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-6 sm:p-8 max-w-xl">
      <div className="flex items-center gap-3.5 mb-6">
        <div className="w-14 h-14 rounded-full bg-teal-100 dark:bg-teal-950/40 flex items-center justify-center text-xl font-bold text-teal-700 dark:text-teal-400 shrink-0">
          {user?.name?.[0]?.toUpperCase() || '?'}
        </div>
        <div>
          <p className="text-[15px] font-semibold">{user?.name}</p>
          <p className="text-[12.5px] text-slate-400 dark:text-slate-500 capitalize">{user?.role?.replace('_', ' ')}</p>
        </div>
      </div>
