...
export default function HotelCard({ hotel }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/40 overflow-hidden hover:shadow-lg hover:border-teal-200 dark:hover:border-teal-900/50 transition flex flex-col">
      <div className="relative h-40 bg-slate-100 dark:bg-slate-900 shrink-0">
        {image ? (
          ...
          <span
            className={`absolute top-2.5 right-2.5 text-[10.5px] font-semibold px-2.5 py-1 rounded-full backdrop-blur ${
              openNow
                ...
                : 'bg-slate-100/90 dark:bg-slate-900/80 text-slate-500 dark:text-slate-400'
            }`}
          >
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-1">
        <div className="flex items-start justify-between gap-2 mb-1.5">
          ...
          {rating !== null && (
            <span className="shrink-0 inline-flex items-center gap-1 text-[12px] font-bold text-teal-700 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/30 px-2 py-0.5 rounded-full">
        ...
        {address && (
          <p className="text-[12.5px] text-slate-500 dark:text-slate-400 mb-2 wrap-break-word">{address}</p>
        )}

        {distance && (
          <div className="flex items-center gap-1.5 text-[12px] text-slate-400 dark:text-slate-500 mb-3">
        ...
        {categories.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {categories.slice(0, 3).map((cat, idx) => (
              <span
                key={idx}
                className="text-[10.5px] font-semibold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full"
              >
        ...
        <div className="mt-auto pt-1">
          {detailsUrl ? (
            <a
              ...
              className="block w-full text-center text-[13px] font-semibold py-2.5 rounded-[10px] border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 hover:border-teal-300 dark:hover:border-teal-900/50 hover:text-teal-600 dark:hover:text-teal-400 transition"
            >
              View Details
            </a>
          ) : (
            <button
              ...
              className="w-full text-center text-[13px] font-semibold py-2.5 rounded-[10px] border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 hover:border-teal-300 dark:hover:border-teal-900/50 hover:text-teal-600 dark:hover:text-teal-400 transition"
            >
        ...
        {expanded && !detailsUrl && (
          <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 text-[12.5px] text-slate-500 dark:text-slate-400 space-y-1">
