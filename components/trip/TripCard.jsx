...
export default function TripCard({ trip, onDelete }) {
  ...
  return (
    <div
      className={`rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-5 sm:p-6 hover:border-teal-200 dark:hover:border-teal-900/50 ${CARD_HOVER}`}
    >
      ...
      <div className="flex flex-wrap gap-1.5 mb-5">
        {trip.interests?.slice(0, 3).map((i) => (
          <span
            key={i}
            className="text-[10.5px] font-semibold text-teal-700 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/30 px-2 py-0.5 rounded-full"
          >
            {i}
          </span>
        ))}
        {trip.interests?.length > 3 && (
          <span className="text-[10.5px] font-semibold text-slate-400 px-2 py-0.5">+{trip.interests.length - 3} more</span>
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        <Link
          ...
          className={`flex-1 text-center text-[13px] font-semibold py-2.5 rounded-[10px] ${BTN_GRADIENT} shadow-[0_2px_8px_rgba(6,182,212,0.18)] hover:shadow-[0_6px_16px_rgba(6,182,212,0.3)] active:scale-[0.98] transition-all duration-200`}
        >
          View itinerary
        </Link>
...
function MiniStat({ label, value }) {
  return (
    <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800 rounded-lg py-2.5">
      <p className="text-[13.5px] font-bold truncate px-1 text-slate-900 dark:text-slate-100">{value}</p>
      <p className="text-[10px] font-medium text-slate-500 dark:text-slate-500 mt-0.5">{label}</p>
    </div>
  );
}
