  ...
  return (
    <div
      className={`relative flex flex-col rounded-2xl p-6 sm:p-8 h-full ${
        isPremium
          ? 'bg-white dark:bg-slate-900/40 border-2 border-teal-300 dark:border-teal-900/60 shadow-xl'
          : 'bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 shadow-sm'
      }`}
    >
      {badge && (
        <span
      ...
      <p className="text-[13px] text-slate-600 dark:text-slate-400 mt-1.5 min-h-9">{description}</p>

      <div className="flex items-end gap-1.5 mt-5 mb-1">
        <span className="text-3xl sm:text-4xl font-bold tracking-tight">{price}</span>
        <span className="text-[13px] text-slate-500 dark:text-slate-400 pb-1.5">{billing}</span>
      </div>
      ...
      <button
        ...
        className={`mt-8 w-full text-center text-[14px] font-bold py-3 rounded-xl transition ${
          isPremium
            ...
            : 'border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 hover:border-teal-300 dark:hover:border-teal-900/50 hover:text-teal-700 dark:hover:text-teal-400'
        }`}
      >
        {buttonLabel}
      </button>
