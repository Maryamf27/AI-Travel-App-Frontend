  ...
  if (loading) {
    return (
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="h-44 rounded-2xl bg-zinc-50 dark:bg-zinc-900 animate-pulse" />
        ))}
      </div>
    );
  }
...
function EmptyState() {
  return (
    <div className="text-center py-16 sm:py-20 px-4 rounded-2xl border border-dashed border-zinc-200 dark:border-zinc-800">
      ...
      <p className="text-[13.5px] sm:text-[14px] text-zinc-500 dark:text-zinc-400 mb-6">
