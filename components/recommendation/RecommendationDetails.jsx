...
export default function RecommendationDetails({ recommendation }) {
  ...
  return (
    <div>
      <div className="mb-8">
        <p className="text-[12px] uppercase tracking-wide text-slate-800 dark:text-slate-500 mb-1">Recommendation for</p>
        <h1 className="text-2xl sm:text-3xl font-semibold wrap-break-word mb-3">{destination}</h1>

        <div className="flex items-center gap-1.5 text-[12.5px] text-slate-800 dark:text-slate-400 mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {interests?.map((i) => (
            <span
              key={i}
              className="text-[12px] font-semibold text-teal-700 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/30 px-3 py-1 rounded-full"
            >
              {i}
            </span>
          ))}
      ...
      {aiResult ? (
        <div className="grid sm:grid-cols-2 gap-4 sm:gap-5">
          {SECTIONS.map(({ key, title, icon }) => {
            ...
            return (
              <div key={key} className="rounded-2xl border border-slate-300 dark:border-slate-800 p-4 sm:p-5">
        ...
        <p className="text-[14px] text-slate-800 dark:text-slate-400">No AI results available for this recommendation.</p>
      )}
    </div>
  );
}
